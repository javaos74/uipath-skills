...
**Key point:** `uipath-openai-agents` is required — it registers the OpenAI Agents runtime factory that enables `uipath run`, `uipath init`, and deployment to detect and execute your agent.
...
# Create UiPath client and set as default for all agents
...
- `uipath init` imports your Python file to introspect the agent
- Avoid side effects at module level (API calls, heavy initialization)
- Use the factory function pattern (def main() -> Agent) to defer setup
...
```bash
uv run uipath run agent '{"messages": "Tell me about this", "query": "What is Python?"}'
```
...
def main() -> Agent:
    ...
    return Agent[WeatherInput](
        name="weather_agent",
        instructions="Use the weather tool to answer questions about the given location.",
        model=MODEL,
        tools=[get_weather],
...
def main() -> Agent:
    ...
    technical_agent = Agent(
        ...
        instructions="You handle technical questions about software.",
        model=MODEL,
    )
    ...
    return Agent[CustomerInput](
        name="triage",
        instructions=(
            "You route customer questions to the right specialist. "
            "Hand off technical questions and billing questions."
        ),
        model=MODEL,
