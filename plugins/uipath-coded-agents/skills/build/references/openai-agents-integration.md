...
> **CRITICAL: Never instantiate `UiPathChatOpenAI` at module level.** `uipath init` imports your file to introspect the agent — module-level LLM clients will fail because auth may not have happened yet. Always use the **factory function pattern** (see below) to defer initialization.
