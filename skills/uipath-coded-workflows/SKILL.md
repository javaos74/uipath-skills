...
| **Call an Integration Service connector** | [references/integration-service.md](references/integration-service.md) — use [uipath-platform skill](../uipath-platform/SKILL.md) first to resolve connector key, connection id, object name, httpMethod, path, and parameter types. **Before writing any Create/Update call:** run Step 1b in that guide to check for `"type": "multipart"` params in the raw metadata file — if found, pass `multipartParameters: new()` to `ExecuteAsync` |
...
Use `IntegrationConnectorService.Create(services.Container).ExecuteAsync(...)` to call any connector (Jira, Salesforce, ServiceNow, Slack, etc.) directly. Requires the connector key, connection id, object name, HTTP method, path, and parameter buckets — all resolved up-front via the `uipath-platform` skill.
