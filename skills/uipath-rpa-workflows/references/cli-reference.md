...
| **Search for activities** | `Bash`: `uip rpa find-activities --query "..." [--tags "..."] [--limit N] --output json --use-studio` | `--query` (required), `--tags`, `--limit` (default 10) |
| **Get default activity XAML (non-dynamic)** | `Bash`: `uip rpa get-default-activity-xaml --activity-class-name "..." --use-studio` | `--activity-class-name` (fully qualified) |
| **Get default activity XAML (dynamic)** | `Bash`: `uip rpa get-default-activity-xaml --activity-type-id "..." [--connection-id "..."] --use-studio` | `--activity-type-id`, `--connection-id` (optional) |
| **List workflow examples** | `Bash`: `uip rpa list-workflow-examples --tags service1,service2 [--prefix "..."] [--limit N] --output json --use-studio` | `--tags` (comma-separated, required), `--prefix` (optional), `--limit` (default 10) |
| **Get workflow example** | `Bash`: `uip rpa get-workflow-example --key "path/to/example.xaml" --use-studio` | `--key` (blob path from list results) |
...
| **Get errors** | `Bash`: `uip rpa get-errors [--file-path "..."] [--skip-validation] --output json --use-studio` | `--file-path` (relative to project dir), `--skip-validation` (use cached errors) |
| **Get JIT type definitions** | `Read` tool on `{projectRoot}/.project/JitCustomTypesSchema.json` | File path |
| **Install/update packages** | `Bash`: `uip rpa install-or-update-packages --packages '[{"id":"..."}]' --use-studio` | `--packages` (JSON array; `version` optional — omit to auto-resolve latest) |
| **Run workflow** | `Bash`: `uip rpa run-file --file-path "..." [--command StartExecution] [--input-arguments '...'] [--log-level ...] --use-studio` | `--file-path` (required), `--command` (`StartExecution` default, `StartDebugging` for UI automation — pauses on error, keeps app open for selector repair), `--input-arguments` (JSON), `--log-level` |

## Project Lifecycle Tools

| Action | How | Key Parameters |
|--------|-----|----------------|
| **Create new project** | `Bash`: `uip rpa new --name "..." [--template-id ...] [--location ...] [--expression-language ...] [--target-framework ...] [--description "..."] --use-studio` | See [Creating New Projects](new-project-setup.md) |
| **Open project in Studio** | `Bash`: `uip rpa open-project [--project-dir "..."]` | `--project-dir --use-studio` (optional) |
| **Close project** | `Bash`: `uip rpa close-project [--project-dir "..."]` | `--project-dir --use-studio` (optional) |

## Studio Management Tools

| Action | How | Key Parameters |
|--------|-----|----------------|
| **List Studio instances** | `Bash`: `uip rpa list-instances --output json` | (none) |
| **Start Studio** | `Bash`: `uip rpa start-studio` | (none) |
| **Focus activity in designer** | `Bash`: `uip rpa focus-activity [--activity-id "..."] --use-studio` | `--activity-id` (IdRef; omit to focus all sequentially) |
...
| Action | How | Key Parameters |
|--------|-----|----------------|
| **Indicate application/screen** | `Bash`: `uip rpa indicate-application [--name "..."] [--parent-id "..." \| --parent-name "..."] [--description "..."] [--activity-class-name "..."] --use-studio` | `--name` (screen name in Object Repository), `--parent-id`/`--parent-name` (application ref), `--description` (optional, describes the screen for documentation) |
| **Indicate UI element** | `Bash`: `uip rpa indicate-element --name "..." --activity-class-name "..." [--parent-id "..." \| --parent-name "..."] [--description "..."] --use-studio` | `--name` (required), `--activity-class-name` (required, e.g. `UiPath.UIAutomation.Activities.TypeInto`), `--parent-id`/`--parent-name` (screen ref), `--description` (optional, describes the element for documentation) |

**UI Automation indication workflow:**
1. First indicate the application/screen: `uip rpa indicate-application --name "MyApp" --description "Main application window" --use-studio` — the user points at the application window
2. Then indicate individual elements within that screen: `uip rpa indicate-element --name "SubmitButton" --activity-class-name "UiPath.UIAutomation.Activities.ClickX" --parent-name "MyApp" --description "Submit button on the login form" --use-studio` — the user points at the element
...
| Action | How | Key Parameters |
|--------|-----|----------------|
| **Get manual test cases** | `Bash`: `uip rpa get-manual-test-cases --output json --use-studio` | (none) |
| **Get manual test steps** | `Bash`: `uip rpa get-manual-test-steps --output json --use-studio` | (none) |

## Integration Service (IS) Tools

| Action | How | Key Parameters |
|--------|-----|----------------|
| **List connectors** | `Bash`: `uip is connectors list [--filter "..."] --output json` | `--filter` (by name/key) |
| **Get connector details** | `Bash`: `uip is connectors get <connector-key> --output json` | `connector-key` (required) |
| **List connections** | `Bash`: `uip is connections list [connector-key] [--connection-id "..."] [--folder-key "..."] --output json` | `connector-key` (optional filter), `--connection-id`, `--folder-key` |
...
| **List connector activities** | `Bash`: `uip is activities list <connector-key> --output json` | `connector-key` (required) |
| **List connector resources** | `Bash`: `uip is resources list <connector-key> [--operation ...] --output json` | `connector-key`, `--operation` (List/Retrieve/Create/Update/Delete/Replace) |
| **Describe resource schema** | `Bash`: `uip is resources describe <connector-key> <object-name> [--operation ...] --output json` | `connector-key`, `object-name`, `--operation` |
...
|---------------|-------|----------|
| `"connection refused"`, `"EPIPE"`, `"pipe not found"` | Studio IPC not available | Run `uip rpa start-studio`, then `uip rpa open-project --project-dir "..." --use-studio` |
...
| Any unrecognized error | Unknown | Check `--verbose` flag on parent: `uip rpa --verbose <command> --use-studio` for debug details, inform the user |
