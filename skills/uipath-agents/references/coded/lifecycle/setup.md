...
2. Run `uip codedagent new my-agent` to scaffold (if no code exists)
3. Run `uv sync` to install dependencies
4. Verify with `uip codedagent --version`
...
7. Run `uip codedagent init` to generate configuration
8. Test with `uip codedagent run main '{"query": "test"}'`
