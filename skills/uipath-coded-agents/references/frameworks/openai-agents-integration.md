...
└── ...                   # Generated by uip codedagents init
```
...
- `"agent"` — the entrypoint name (used with `uip codedagents run agent`)
...
**Key point:** `uipath-openai-agents` is required — it registers the OpenAI Agents runtime factory that enables `uip codedagents run`, `uip codedagents init`, and deployment to detect and execute your agent.
...
> **CRITICAL: Never instantiate `UiPathChatOpenAI` at module level.** `uip codedagents init` imports your file to introspect the agent — module-level LLM clients will fail because auth may not have happened yet. Always use the **factory function pattern** (see below) to defer initialization.
...
1. `uip codedagents init` loads all registered middleware via Python entry points
...
### Troubleshooting `uip codedagents init`
...
- `uip codedagents init` imports your Python file to introspect the agent
...
- **Fix: Always use the factory function pattern** (`def main() -> Agent`) to defer LLM setup:
  ```python
  # BAD — fails during uip codedagents init:
  client = UiPathChatOpenAI(model_name=MODEL)
  _openai_shared.set_default_openai_client(client.async_client)
...
```bash
uip codedagents run agent '{"messages": "Tell me about this", "query": "What is Python?"}'
```
