...
**Key point:** `uipath-langchain` is required — it registers the LangGraph runtime factory that enables `uip codedagents run`, `uip codedagents init`, and deployment to detect and execute your graph.
...
> **CRITICAL: Never instantiate LLM clients at module level.** `uip codedagents init` imports your file — module-level LLM clients will fail because auth may not have happened yet. Always create LLM instances inside graph nodes or functions.
...
Define `GraphInput` and `GraphOutput` as Pydantic models. These become the agent's schema for `uip codedagents init` and deployment:
...
- `uip codedagents init` imports your Python file to introspect the graph
...
```bash
# With langgraph.json
uip codedagents run agent '{"query": "What is Python?"}'

# With uipath.json functions (using the function name)
uip codedagents run graph '{"query": "What is Python?"}'
```
