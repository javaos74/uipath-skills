...
**Key point:** `uipath-llamaindex` is required — it registers the LlamaIndex runtime factory that enables `uip codedagents run`, `uip codedagents init`, and deployment to detect and execute your workflow.
...
> **CRITICAL: Never instantiate LLM clients at module level.** `uip codedagents init` imports your file — module-level LLM clients will fail because auth may not have happened yet. Always create LLM instances inside workflow steps or functions.
...
- `uip codedagents init` imports your Python file to introspect the workflow
